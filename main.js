...
var Settings = class {
  constructor() {
    ...
    const templateHeading = this.settings.newSettingBuilder().key("templateHeading").name("Template heading").desc("Which heading from your template should the todos go under").dropdown("none");
    this.deleteOnComplete = this.settings.newSettingBuilder().key("removeTagsForTasksPlugin").name("Remove tags from reminder title").desc("If checked, tags(#xxx) are removed from the reminder list view and notification.").tag(TAG_RESCAN).toggle(false).onAnyValueChanged((context) => {
      context.setEnabled(reminderFormatSettings.enableTasksPluginReminderFormat.value);
    }).build(new RawSerde());
  ...
  setRolloverTemplateHeadingSelector(val) {
    if (this.setRolloverTemplateHeadingSelectorHasBeenSet) {
      return;
    }
    const rolloverTemplateHeadingBuilder = this.settings.newSettingBuilder().key("templateHeading").name("Template heading").desc("Which heading from your template should the todos go under").dropdown("none");
    this.rolloverTemplateHeading = rolloverTemplateHeadingBuilder.build(new ReminderFormatTypeSerde());
    val.forEach((f) => rolloverTemplateHeadingBuilder.addOption(`${f}`, f));
    this.settings.newGroup("Rollover Template").addSettings(this.rolloverTemplateHeading);
...
var ReminderSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  async getTemplateHeadings() {
    const periodicNoteSetting = (0, import_obsidian_daily_notes_interface.getDailyNoteSettings)();
    const template = periodicNoteSetting.template;
    if (!template)
      return [];
...
var ObsidianManagerPlugin = class extends import_obsidian5.Plugin {
  ...
  async rollover(file) {
    ...
    if (!this.isDailyNotesEnabled()) {
      new import_obsidian5.Notice("ObsidianManagerPlugin unable to rollover unfinished todos: Please enable Daily Notes, or Periodic Notes (with daily notes enabled).", 1e4);
    } else {
...
${todos_today.join("\n")}`;
      ...
      if (message.length > 0) {
        new import_obsidian5.Notice(message, 4e3 + message.length * 3);
      }
