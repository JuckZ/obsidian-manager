...
var Settings = class {
  constructor() {
    ...
    this.settings.newGroup("Rollover TODOs").addSettings(this.rolloverTemplateHeading, this.deleteOnComplete, this.removeEmptyTodos);
...
var InsertLinkModal = class extends import_obsidian5.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian5.Setting(contentEl).setName("Link text").addText((text) => text.setValue(this.linkText).onChange((value) => {
      this.linkText = value;
    }));
    new import_obsidian5.Setting(contentEl).setName("Link URL").addText((text) => text.setValue(this.linkUrl).onChange((value) => {
      this.linkUrl = value;
    }));
...
var ObsidianManagerPlugin = class extends import_obsidian6.Plugin {
  ...
  async rollover(file) {
    ...
    if (!this.isDailyNotesEnabled()) {
      new import_obsidian6.Notice("ObsidianManagerPlugin unable to rollover unfinished todos: Please enable Daily Notes, or Periodic Notes (with daily notes enabled).", 1e4);
    } else {
...
${todos_today.join("\n")}`;
      ...
      if (message.length > 0) {
        new import_obsidian6.Notice(message, 4e3 + message.length * 3);
      }
  ...
  setupUI() {
    ...
    this.addRibbonIcon("dice", "Sample Plugin", () => {
      new import_obsidian6.Notice("This is a notice!");
    });
  }
  watchVault() {
    [
